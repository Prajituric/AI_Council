...
function autoToPptx(text,title){const slides=[];let cur=null;text.split('\n').forEach(line=>{if(line.startsWith('## ')||line.startsWith('### ')){if(cur)slides.push(cur);cur={title:line.replace(/^#+\s*/,'').replace(/[✅💡⚠🎯]/g,'').trim(),bullets:[],notes:''};}else if(cur&&(line.startsWith('- ')||line.startsWith('* '))){cur.bullets.push(line.slice(2).trim());}else if(cur&&line.trim()&&!line.startsWith('```')){cur.notes+=line.trim()+' ';}});if(cur)slides.push(cur);return{title:title||'AI Council',subtitle:'Generated by AI Council',slides:slides.slice(0,15)};}
function buildPptx(cfg){if(typeof PptxGenJS==='undefined'){alert('PptxGenJS not loaded.');return;}const prs=new PptxGenJS();prs.layout='LAYOUT_WIDE';prs.author='AI Council';const ts=prs.addSlide();ts.background={color:'0f0f13'};ts.addText(cfg.title||'Presentation',{x:.5,y:1.5,w:12,h:1.5,fontSize:40,color:'E8E8F2',bold:true,align:'center'});if(cfg.subtitle)ts.addText(cfg.subtitle,{x:.5,y:3.2,w:12,h:.6,fontSize:20,color:'8888AA',align:'center'});ts.addText('AI Council',{x:.5,y:6.8,w:12,h:.3,fontSize:12,color:'6366F1',align:'center'});(cfg.slides||[]).forEach((slide,i)=>{const s=prs.addSlide();s.background={color:'0f0f13'};s.addText(slide.title||`Slide ${i+1}`,{x:.5,y:.3,w:12,h:.8,fontSize:26,color:'E8E8F2',bold:true});s.addShape(prs.ShapeType.rect,{x:.5,y:1.2,w:12,h:.03,fill:{color:'6366F1'}});if(slide.bullets?.length){const bt=slide.bullets.slice(0,8).map(b=>({text:b,options:{bullet:{code:'2022'},fontSize:18,color:'E8E8F2',paraSpaceBefore:10}}));s.addText(bt,{x:.5,y:1.4,w:12,h:4.5,valign:'top'});}if(slide.notes)s.addNotes(slide.notes);s.addText(`${i+2}`,{x:12.3,y:6.9,w:.5,h:.3,fontSize:11,color:'444466',align:'right'});});prs.writeFile({fileName:`AI_Council_${Date.now()}.pptx`});}
